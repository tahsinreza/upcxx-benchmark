...
int main(int argc, char *argv[]) {
  ...
  for (uint64_t i = 0; i < global_send_count; i++) {
    
    if (i < random_global_table_indices.size()) {    
  ...
  if (upcxx::rank_me() == 0) {
    ...
    //std::cout << "Rank " << upcxx::rank_me() << ", #Local updates: " << 
    //  DistrMap::get_local_recv_count() << std::endl;
  }
