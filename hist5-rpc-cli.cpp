...
int main(int argc, char *argv[]) {
  ...
  uint64_t ranks_per_node = upcxx::local_team().rank_n();
  ...
  uint64_t log_global_table_size = std::stoull(argv[1], NULL, 10); // 28	
  uint64_t global_update_count = std::stoull(argv[2], NULL, 10); // 1600000000

  uint64_t global_table_size = ((uint64_t)1) << log_global_table_size; // e.g., 2^30
  uint64_t local_table_size = (global_table_size / upcxx::rank_n()) + 1; 

  uint64_t local_update_count = global_update_count / upcxx::rank_n(); 	 	  
